...
<body>	 
	...
	<div class="row-fluid">
		...
		<div class="panel-heading"><div lang="en" class="en_content">Search for locations of water gauge record stations in Germany by water, river or canal</div><div lang="de" style="display: none;"class="de_content">Suche nach Wasserstands-Messstationen in Deutschland nach Seen, Flüssen und Kanälen</div></div>				<table class="table table-hover ">
							<tr>
							<td><div lang="en" class="en_content">Water/River/Canal: </div><div lang="de" style="display: none;"class="de_content">See/Fluss/Kanal: </div></td>								<td>
									<form id="search-form" class="input-group" action="WSV_pegel_results.php" method="get">
										<select id="search-query" type="text" class="form-control" autocomplete="off"  name="waters">
						...
						<tr bgcolor=#F5F5F5>
							<td><div lang="en" class="en_content">Results: </div><div lang="de" style="display: none;"class="de_content">Ergebnisse: </div></td>
								<td>
									<?php
										$waters=$_GET['waters'];
										error_reporting(0);							
										if(empty($_GET['waters'])){ 
											$url = 'https://www.pegelonline.wsv.de/webservices/rest-api/v2/stations.json';										
										} else {
										$url='https://www.pegelonline.wsv.de/webservices/rest-api/v2/stations.json?waters='.$waters.'&includeTimeseries=true&includeCurrentMeasurement=true';
										}									
										$curl = curl_init();
										//$headers = array(
										...
										echo 'Total-Number of records ' .$Anzahl;									
										$my_fila = 'wsv.json';										
										$handle = fopen($my_fila, 'w') or die('Cannot open file:  '.$my_file);										fwrite($handle, json_encode($response));
										$string = file_get_contents("wsv.json");

										$jsonRS = json_decode ($string,true);
										$timestamp = filemtime($my_fila);
